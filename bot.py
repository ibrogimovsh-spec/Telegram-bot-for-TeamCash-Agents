    ...
    for method in settings.payment_methods:
        ...
        if method.url:
            try:
                ...
                logging.exception("Неверный шаблон ссылки %s", method.key)
                button = InlineKeyboardButton(
                    text=uniform_button_text(method.name),
                    callback_data=f"bank:missing:{method.key}",
                    style="success",
                )
        else:
            button = InlineKeyboardButton(
...
async def ensure_ready(message: Message) -> UserProfile | None:
...
async def accept_deposit_amount(
    ...
    caption = translate(
        language,
        f"💳 <b>Пополнение</b>\n➡️ Ваш ID: <code>{html.escape(str(data['account_id']))}</code>\n💰 Сумма к оплате: <b>{format_money(total_minor)}</b>\n\n⏰ У вас есть {settings.payment_timeout_seconds // 60} минут на оплату.\n🧾 Чек отправьте после оплаты.\n\nВыберите банк кнопкой под QR-кодом.",
        f"💳 <b>Deposit</b>\n➡️ Your ID: <code>{html.escape(str(data['account_id']))}</code>\n💰 Amount due: <b>{format_money(total_minor)}</b>\n\n⏰ You have {settings.payment_timeout_seconds // 60} minutes to pay.\n🧾 Send a receipt after payment.\n\nChoose a bank using a button below.",
        f"💳 <b>Толуктоо</b>\n➡️ Сиздин ID: <code>{html.escape(str(data['account_id']))}</code>\n💰 Төлөнүүчү сумма: <b>{format_money(total_minor)}</b>\n\n⏰ Төлөө үчүн {settings.payment_timeout_seconds // 60} мүнөт бар.\n🧾 Төлөгөндөн кийин чекти жөнөтүңүз.\n\nQR-коддун астындагы баскыч менен банкты тандаңыз.",
    )
    caption += translate(
        language,
...
async def deposit_begin(message: Message, state: FSMContext) -> None:
...
async def withdrawal_begin(message: Message, state: FSMContext) -> None:
    if await ensure_ready(message) is None:
        return
...
async def user_operations(message: Message) -> None:
    profile = await ensure_ready(message)
    if profile is None:
        return
    language = profile.language or "ru"
